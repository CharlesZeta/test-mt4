            ...
            if parsed_json.get("desc") == "QUOTE_DATA":
                try:
                    ...
                    if isinstance(qd, dict) and "bid" in qd:
                        _update_quote_cache(sym, qd["bid"], qd["ask"],
                                            spread=parsed_json.get("spread"), ts=parsed_json.get("ts"))
                except: pass
            elif parsed_json.get("bid") is not None and parsed_json.get("ask") is not None:
                _update_quote_cache(sym, parsed_json["bid"], parsed_json["ask"],
                                    spread=parsed_json.get("spread"), ts=parsed_json.get("ts"))
    with history_lock:
...
def _process_tick(tick):
    ...
    record = {
        ...
        "parsed": {"desc": "QUOTE_DATA", "spread": spread, "ts": tt,
...
def tick_ren():
    try:
        ...
        count = sum(1 for t in ticks if _process_tick(t))
...
def api_hist():
    ...
    try:
        ...
        with conn.cursor() as cur:
            if agg == "raw":
                cur.execute(
                    "SELECT symbol,bid,ask,spread,mid,tick_time,received_at "
                    "FROM tick_logs WHERE symbol=%s AND tick_time>=%s AND tick_time<=%s "
                    "ORDER BY tick_time ASC LIMIT %s",
                ...
                return jsonify({"error": f"unsupported agg: {agg}"}), 400
        conn.close()
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "MIN(tick_time) AS first_ts, MAX(tick_time) AS last_ts "
...
def receive_tick():
    try:
        ticks = request.json
        if not isinstance(ticks, list): ticks = [ticks]
        for t in ticks: _process_tick(t)
        return '', 204
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def api_latest_status():
    ...
    if sf:
        with quotes_lock:
            ...
            if q:
                latest_quote = {"bid": q["bid"], "ask": q["ask"], "symbol": sf,
                    "spread": q["spread"], "ts": q["ts"], "received_at": q["received_at"]}
    with history_lock:
...
def api_quotes():
    with quotes_lock:
        return jsonify({sym: {"bid":q["bid"],"ask":q["ask"],"spread":q["spread"],"ts":q["ts"],"received_at":q["received_at"]}
...
for(let v=tI;v<=tA+ns*.5;v+=ns)tk.push(v);return{tI,tA,ns,tk}}
...
const G=cv('--green'),R=cv('--red'),c2=up?G:R;
...
async function chkDB(){try{const r=await fetch('/api/debug/db'),d=await r.json();
const tag=$('dbTag');if(!tag)return;
if(d.db_ok){tag.className='db-tag ok';tag.innerText='DB '+d.total_rows}
else{tag.className='db-tag err';tag.innerText='DB ✕'}}
catch(e){const tag=$('dbTag');if(tag){tag.className='db-tag err';tag.innerText='DB ✕'}}}
tick();setInterval(tick,1000);rf();setInterval(rf,2000);setInterval(chkS,2000);
fK();setInterval(fK,2000);chkDB();setInterval(chkDB,15000);
